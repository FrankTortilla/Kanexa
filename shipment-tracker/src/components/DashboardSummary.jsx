...
export default function DashboardSummary({ shipments, isWarehouse, onCardClick, metrics }) {
  ...
  const pickup     = shipments.filter(s => s.status === 'Pickup').length;
  ...
  // Flow tiles: standard shipping sequence
  const flowCards = [
  ...
  // Exception tiles: fulfillment alternatives
  const exceptionCards = [
    { key: 'pickup',    label: 'Pickup',    value: pickup,    color: 'var(--accent-warning)',    glow: 'var(--accent-warning-glow)'    },
    { key: 'cancelled', label: 'Cancelled', value: cancelled, color: 'var(--accent-danger)',     glow: 'var(--accent-danger-glow)'     },
  ];

  // Total tile (office only)
  const totalCard = { key: 'total', label: 'Total Active', value: total, color: 'var(--text-primary)', glow: 'transparent' };
  ...
  return (
    <div className="no-print summary-cards-scroll" style={{
      ...
      {/* Flow tiles: standard shipping sequence */}
      {flowCards.map(card => <StatCard key={card.key} card={card} fontSize={fontSize} labelSize={labelSize} onCardClick={onCardClick} />)}

      {/* Divider: flow vs exceptions */}
      <div style={{ width: '1px', alignSelf: 'stretch', background: 'rgba(255,255,255,0.1)', margin: '0 4px', flexShrink: 0 }} />

      {/* Exception tiles: Pickup + Cancelled */}
      {exceptionCards.map(card => <StatCard key={card.key} card={card} fontSize={fontSize} labelSize={labelSize} onCardClick={onCardClick} />)}

      {/* Total Active tile (office only) */}
      {!isWarehouse && <StatCard card={totalCard} fontSize={fontSize} labelSize={labelSize} onCardClick={onCardClick} />}
...
function StatCard({ card, fontSize, labelSize, onCardClick }) {
  return (
    <div
